...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            for (int i = startIndexInclusive; i < endIndexExclusive; i++) value += 1.0 / input[i];
            // Para hacer: completar el cuerpo de este método para realizar la suma de los recíprocos de la sección del arreglo
            // que esta tarea es responsable de procesar
        }
    }
    ...
    protected static double parArraySum(final double[] input) {
        ...
        // System.out.println("Longitud del arreglo " + input.length);

        // if (input.length == 2_000_000) {
        //     System.out.println("Valor de suma inversa: " + (left.getValue() + right.getValue()));
        // }
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for (int i = 0; i < numTasks; i++) {
            int istart = getChunkStartInclusive(i, numTasks, input.length);
            int iend = getChunkEndExclusive(i, numTasks, input.length);
            tasks[i] = new ReciprocalArraySumTask(istart, iend, input);
        }
